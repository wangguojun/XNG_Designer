{
	...
	// Copyright (c) 2005-2010 redefinery (Jeffrey R. Almasol). All rights reserved.
	...
	function rd_ScriptLauncher(thisObj)
	{
		...
		rd_ScriptLauncherData.scriptName = "小年糕脚本管理器";
		rd_ScriptLauncherData.scriptTitle = rd_ScriptLauncherData.scriptName + " v1.0";
		rd_ScriptLauncherData.scriptPath = "";
		rd_ScriptLauncherData.scriptFiles = new Array();
		rd_ScriptLauncherData.logoFile="AEScripts.png";
		rd_ScriptLauncherData.strScriptsFolder = {en: "选择文件夹"};
		rd_ScriptLauncherData.strRefreshList = {en: "刷新"};
		rd_ScriptLauncherData.strRun = {en: "运行"};
		rd_ScriptLauncherData.strHelp = {en: "有问题请点我"};
		rd_ScriptLauncherData.strErrNoScriptsPath = {en: "无法打开文件夹,因为无法找到."};
		rd_ScriptLauncherData.strErrMissingFile = {en: "无法找到选定的脚本."};
		rd_ScriptLauncherData.strMinAE80 = {en: "此脚本运行环境在cs3或更高."};
		rd_ScriptLauncherData.strHelpText = 
		{
			en: "Copyright (c) 2018 Limbo Eric \n" +
			"All rights reserved.\n" +
			"\n" +
			"本脚本主要用于管理小年糕设计师AE脚本/插件，也可以将其他脚本/插件放置于本脚本(xngDesignerTools/script)目录下，一同管理。\n" +
			"\n" +
			"初次使用请点击左下角“选择文件夹”，找到您的安装目录，找到xngDesignerTools文件夹下的script文件夹，点击确认，程序会自动扫描script目录下的所有.jsx/.jsxbin脚本。\n默认路径为C:/xngDesignerTools/script"
		};
		...
		function rd_ScriptLauncher_buildUI(thisObj)
		{
			...
			if (pal != null)
			{
				...
				"group { \
					...
					header: Group { \
						...
						help: Button { text:'" + rd_ScriptLauncher_localize(rd_ScriptLauncherData.strHelp) +"', maximumSize:[120,25], alignment:['right','center'] }, \
					}, \
													logoBtn: " + 
			((File(rd_ScriptLauncherData.logoFile).exists) ? 
				...
				("Button {text: '点此寻求帮助 (logo文件错误)'}, "))  // use normal button instead if image got lost...
			+ "\
